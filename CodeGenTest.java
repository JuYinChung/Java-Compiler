...
//		show(program);  //It may be useful useful to show this here if code generation fails
	...
	public void emptyProg() throws Exception {
		...
		String input =prog+"{image y[130,130];\n"
				
				+ "show 130;}";
		byte[] bytecode = genCode(input);
		String[] commandLineArgs = {"/home/chungjuyin/eclipse-workspace/PLPHomework/3.png","/home/chungjuyin/eclipse-workspace/PLPHomework/4.png"};
	...
	public void integerLit() throws Exception {
		...
		String input = prog + "{image im[256,256];"
				+ "filename f; input f from @0; "
				+ "int x;int y; x := 0; y := 0;"
				+ "while (x < width(im)){"
				+ "  y := 0; while (y < height(im)){"
				+ "im[x,y] := <<15,255,0,0>>; int z; z := im[x,y];y := y + 1;"
				+ " };"
				+ "x := x + 1;"
				+ "};"
				+ "write im to f"
				+ ";}";
		byte[] bytecode = genCode(input);		
		String[] commandLineArgs = {"1.png","2.png"}; //create command line argument array to initialize params, none in this case		
		runCode(prog, bytecode, commandLineArgs);	
		show("Log:\n"+RuntimeLog.globalLog);
		assertEquals("entering main;leaving main;",RuntimeLog.globalLog.toString());
	}
	...
	public void booleanLit() throws Exception {
		...
		String input = prog + "{image y; image copy[128,256]; input y from @ 0 ; show y; copy := y; show copy;sleep(5000);} ";	
		byte[] bytecode = genCode(input);		
		String[] commandLineArgs = {"3.png"}; //create command line argument array to initialize params, none in this case		
		runCode(prog, bytecode, commandLineArgs);	
		show("Log:\n"+RuntimeLog.globalLog);
		assertEquals("entering main;leaving main;",RuntimeLog.globalLog.toString());
	}
	...
	public void BinaryExpressionLit() throws Exception {
		...
		String input = prog + "{show 1+2*3+4;} ";	
		byte[] bytecode = genCode(input);		
		String[] commandLineArgs = {}; //create command line argument array to initialize params, none in this case		
		runCode(prog, bytecode, commandLineArgs);	
		show("Log:\n"+RuntimeLog.globalLog);
	...
	public void FunctionExpressionWithArgLit9() throws Exception {
		...
		byte[] bytecode = genCode(input);		
		String[] commandLineArgs = {"1.png"}; //create command line argument array to initialize params, none in this case		
		runCode(prog, bytecode, commandLineArgs);	
		show("Log:\n"+RuntimeLog.globalLog);
		assertEquals("entering main;leaving main;",RuntimeLog.globalLog.toString());
	}
	...
	public void pixelConstructorTest() throws Exception {
		...
		String input = prog + "{int i; i := 0; while(i < 2){i:=i+1; show 3;}; }";	
		byte[] bytecode = genCode(input);		
		String[] commandLineArgs = {}; //create command line argument array to initialize params, none in this case		
		runCode(prog, bytecode, commandLineArgs);
	...
	public void AssignmentTest() throws Exception {
		...
		String input = prog + "{ image im[256,256]; "
				+ "filename f; input f from @0; int x; int y; x := 0; y := 0; "
				+ "while (x < width(im))"
				+ "{ "
				+ " y := 0; "
				+ "	while (y < height(im))"
	...
	public void pixelSelectorTest() throws Exception {
		...
		String input = prog + "{image i; input i from @ 0; show i[128, 128];} ";	
		byte[] bytecode = genCode(input);		
		String[] commandLineArgs = {"1.png"}; //create command line argument array to initialize params, none in this case		
		runCode(prog, bytecode, commandLineArgs);	
