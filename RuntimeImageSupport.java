...
public class RuntimeImageSupport {
	...
	public static BufferedImage deepCopy(BufferedImage image) throws IOException {
		if(image == null)
	...
	//(sample,color, x, y,image) 
	public static String updatePixelColorSig = "(IIII" + ImageDesc + ")V";
	public final static void updatePixelColor(int sampleVal,int color, int x, int y,
			BufferedImage image) {
		if (x < 0 || x >= image.getWidth() || y < 0 || y >= image.getHeight()) {
			System.out.println("pixel out of bounds");
			return;
	...
	public static String makeFrameSig = "(" + ImageDesc + "II)" + JFrameDesc;
	/**
	 ...
	 */
	public static final JFrame makeFrame(BufferedImage image,int i,int j)
			throws InvocationTargetException, InterruptedException {
		JFrame frame = new JFrame();
		frame.setDefaultCloseOperation(EXIT_ON_CLOSE);
		System.out.println("frameX = " + i + " frameY = " + j);
		frame.setSize(i,j);
